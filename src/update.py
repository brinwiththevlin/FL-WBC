...
class LocalUpdate(object):
    def __init__(
        ...
        self.logger = logger
        self.mal = mal
        self.weight_tampering = args.weight_tampering
        if (
            args.dataset == "femnist"
            or args.dataset == "cifar10_extr_noniid"
            or args.dataset == "miniimagenet_extr_noniid"
            or args.dataset == "mnist_extr_noniid"
            or args.dataset == "HAR"
            or args.dataset == "HAD"
        ):
            if dataset_test is None or idxs_test is None:
                print(
                    "error: femnist and cifar10_extr_noniid need dataset_test and idx_test in LocalUpdate!\n"
                )
            self.trainloader, self.validloader, self.testloader = (
                self.train_val_test_femnist(
                    dataset, list(idxs), dataset_test, list(idxs_test)
                )
            )
        else:
    ...
    def train_val_test_femnist(self, dataset, idxs, dataset_test, idxs_test):
        ...
        trainloader = DataLoader(
            DatasetSplit(dataset, idxs_train, self.mal, self.mal_target),
            batch_size=self.args.local_bs,
            shuffle=True,
        )
        validloader = DataLoader(
            DatasetSplit(dataset, idxs_val, self.mal, self.mal_target),
            batch_size=int(len(idxs_val)),
            shuffle=False,
        )
        testloader = DataLoader(
            DatasetSplit(dataset_test, idxs_test, self.mal, self.mal_target),
            batch_size=40,
            shuffle=True,
        )
    ...
    def mal_loader(self, dataset, idxs, Y):
        malloader = DataLoader(
            DatasetSplit(dataset, idxs, Y), batch_size=self.args.local_bs, shuffle=True
        )
        return malloader
    ...
    def update_weights(self, model, global_round, device):
        ...
        if self.args.optimizer == "sgd":
            ...
            #                             weight_decay=1e-4)
        if self.mal is True:
            if self.weight_tampering == "large_neg":
                tamper_weights_large_negative(model)
            elif self.weight_tampering == "reverse":
                tamper_weights_reverse(model)
            elif self.weight_tampering == "random":
                tamper_weights_random(model)
            elif self.weight_tampering == "none":
                pass
            else:
                            ...
                            for name, p in model.named_parameters():
                                if "weight" in name:
                                    ...
                                    pertubation = np.random.laplace(
                                        0,
                                        self.args.pert_strength,
                                        size=grad_tensor.shape,
                                    ).astype(np.float32)
                                    pertubation = np.where(
                                        abs(grad_diff) > abs(pertubation),
                                        0,
                                        pertubation,
                                    )
                                    p.data = torch.from_numpy(
                                        p.data.cpu().numpy()
                                        + pertubation * self.args.lr
                                    ).to(device)
                        for name, p in model.named_parameters():
                            if "weight" in name:
...
def mal_inference(args, model, test_dataset, mal_X_list, mal_Y):
    ...
    criterion = nn.NLLLoss().to(device)
    malloader = DataLoader(
        DatasetSplit(test_dataset, mal_X_list, mal_Y),
        batch_size=args.local_bs,
        shuffle=True,
    )

    for batch_idx, (images, labels, labels_true) in enumerate(malloader):
        images, labels, labels_true = (
            images.to(device),
            labels.to(device),
            labels_true.to(device),
        )
        ...
        confidence_sum += sum(
            (F.softmax(outputs.data.detach(), dim=1).cpu().data)[idx_list, label_list]
        )
