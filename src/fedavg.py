...
if __name__ == "__main__":
    ...
    torch.manual_seed(12345)  # cpu
    torch.cuda.manual_seed(12345)  # gpu
    ...
    # BUILD MODEL
    data_model = { "fmnist": CNNFashion_Mnist, "cifar": CNNCifar, "mnist": CNNMnist }
    if args.model == "cnn":
        # Convolutional neural netork
        global_model = data_model[args.dataset](args=args)
    else:
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for idx in idxs_users:
            ...
            if mal_user == True:
                print(
                    "user {}, loss {}, acc {}, mal loss {}, mal acc {}".format(
                        idx, loss, acc, mal_loss, mal_acc
                    )
                )
            else:
        ...
        if (epoch + 1) % print_every == 0:
            ...
            print(
                "Global model Malicious Accuracy: {:.2f}%, Malicious Loss: {:.2f} , Outputs: {}\n".format(
                    100 * mal_acc, 100 * mal_loss, mal_out
                )
            )
